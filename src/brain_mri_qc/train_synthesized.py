...
def prepare_abide_data(data_root, tsv_path, site_list):
    ...
    def binarize(row):
        if pd.isna(row['score']) or str(row['confidence']).lower() == 'exclude': return None
    ...
    for scan in Path(data_root).rglob("mprage.nii.gz"):
        ...
        if any(site.upper() in path_str.upper() for site in site_list):
            ...
            if match:
                ...
                if sid in label_info: matched_files.append({"image": path_str, "label": label_info[sid]})
...
sampler = WeightedRandomSampler([class_weights[l] for l in train_labels], len(train_labels), replacement=True)
